<?php 
	...
	class ProductController extends Controller{
		...
		public function detail(){
			$id = isset($_GET["id"])&&is_numeric($_GET["id"]) ? $_GET["id"] : 0;
			//goi ham trong model de lay 1 ban ghi
			$record = $this->fetch_recipe($id);
			//tao bien $formAction de dieu phoi action cua form
			$formAction = "index.php?area=backend&controller=Product&action=doUpdateRecipe&id=$id";
			//goi view, truyen du lieu ra view
			$this->renderHTML("Views/Backend/RecipeView.php",array("record"=>$record,"formAction"=>$formAction));
		}
	}
 ?>